...
# Дом для комнаты уровня 2 (масштаб под полную высоту экрана — верх дома вверху)
_house_img = load_image("assets/level_2/house.png", 1.0)
if _house_img:
    _hw, _hh = _house_img.get_width(), _house_img.get_height()
    house_sprite = pygame.transform.scale(_house_img, (int(_hw * SCREEN_HEIGHT / _hh), SCREEN_HEIGHT))
else:
    house_sprite = None
# Магазин внутри дома (уровень 2)
shop_sprite = load_image("assets/level_2/shop.png", 1.0)
# Монета уровня 2 (+1 к деньгам за штуку)
...
class GameObject:
    ...
    def draw(self, screen):
        if not self.collected:
            ...
            if self.object_type in ["key", "crystal", "coin"]:
    ...
    def update(self, player_x=None, player_y=None):
        ...
        elif self.object_type == "worm" and not self.defeated:
            if self.attack_cooldown < self.worm_attack_interval:
                self.attack_cooldown += 1
            if player_x is not None and player_y is not None:
                worm_center_x = self.x + self.width / 2
                self.boss_direction = "right" if player_x > worm_center_x else "left"
                dx = player_x - self.x
                dy = player_y - self.y
                distance = math.sqrt(dx * dx + dy * dy)
...
class Scene:
    ...
    def draw(self, screen):
        ...
        elif self.name == "level2_house_scene":
            # Дом на всю высоту экрана (верх дома касается верха экрана)
            if house_sprite:
                house_w = house_sprite.get_width()
...
def main():
    ...
    in_shop = False  # Игрок внутри дома (магазин) на уровне 2
    # Взрыв бомбы (F — бросить)
    ...
    while running:
        for event in pygame.event.get():
            if event.type == pygame.QUIT:
                ...
                if event.key == pygame.K_e and in_shop:
                    in_shop = False
                    shop_exit_cooldown = 25  # ~0.4 сек — не реагировать на E у входа
        ...
        if not game_over and not victory:
            ...
            # В магазине не двигаемся — только E для выхода (обработка в event loop)
            if not in_shop:
            ...
            # Лава на уровне 2: пока стоишь в луже — 5 урона раз в ~1 сек; вышел — урон прекращается
            if current_level == 2 and not in_shop and not game_over and not victory:
                in_lava = False
                hx, hy, hw, hh = vasily.get_defense_hitbox(hero_sprite_width, hero_sprite_height)
                for obj in current_scene_obj.objects:
                    if obj.object_type == "lava":
                        if (hx < obj.x + obj.width and hx + hw > obj.x and
                                hy < obj.y + obj.height and hy + hh > obj.y):
                            in_lava = True
                            break
                if in_lava:
                    if lava_damage_cooldown <= 0:
                        if vasily.take_damage(LAVA_DAMAGE):
                            game_over = True
                        else:
                            print("Лава нанесла 5 урона!")
                        lava_damage_cooldown = LAVA_DAMAGE_INTERVAL
                    else:
                        lava_damage_cooldown -= 1
                else:
                    lava_damage_cooldown = 0
            ...
            for obj in current_scene_obj.objects:
                if obj.object_type == "boss":
                    ...
                    if not obj.defeated and obj.attack_cooldown >= obj.worm_attack_interval:
                        worm_attack = obj.get_attack_area(vasily.x)
                        if worm_attack:
                            ax, ay, aw, ah = worm_attack
                            if vasily.attacking and vasily.has_sword:
            ...
            for obj in current_scene_obj.objects:
                ...
                if (distance_x < 80 and distance_y < 80 and not obj.collected and not in_shop):
                    # Подсказки по типу объекта (в магазине E только выходит, деньги не начисляются)
                    ...
                    if obj.object_type in ["sword_in_stone", "trident_in_stone"] and keys[pygame.K_e]:
                        ...
                        vasily.money += 1
                        obj.collected = True
                        print(f"Василий подобрал монету! Деньги: {vasily.money}")
                ...
                if multiplayer_mode and vasily2 and not obj.collected:
                    ...
                    if (distance_x2 < 80 and distance_y2 < 80):
                        if obj.object_type in ["sword_in_stone", "trident_in_stone"] and keys[pygame.K_e]:
                            ...
                            victory = True
                    # Первый игрок у двери (в мультиплеере — только если второй не в зоне, чтобы не открыть дважды)
                    if obj.object_type == "door" and not (distance_x2 < 80 and distance_y2 < 80) and vasily.keys >= 3 and boss_defeated and keys[pygame.K_e]:
                        if current_level == 1:
                            current_level = 2
                            if background_level2:
                                background_far = background_level2
                    ...
                    if not obj.defeated and obj.attack_cooldown >= obj.attack_interval:
                        boss_attack_area = obj.get_attack_area(vasily.x)
                        if boss_attack_area:
                            attack_x, attack_y, attack_w, attack_h = boss_attack_area
                            if vasily.attacking and vasily.has_sword:
                                if vasily.weapon_type == "trident" and hero_trident_attack_sprite:
                            ...
                            if (hero_hitbox_x < attack_x + attack_w and hero_hitbox_x + hero_hitbox_w > attack_x and
                                hero_hitbox_y < attack_y + attack_h and hero_hitbox_y + hero_hitbox_h > attack_y):
                                if vasily.take_damage(15):
                                    game_over = True
                                else:
                                    print("Босс атаковал Василия! -15 HP")
                                obj.attack_cooldown = 0
            
            # Взрыв бомбы (урон по области)
            if explosion_active:
                explosion_timer += 1
                if explosion_timer == 10 and not explosion_damage_dealt:
                    for obj in current_scene_obj.objects:
                        if obj.object_type in ("boss", "worm") and not obj.defeated:
                            ...
                            dist = math.sqrt((cx - explosion_x) ** 2 + (cy - explosion_y) ** 2)
                            if dist < BOMB_EXPLOSION_RADIUS:
                                if obj.take_damage(BOMB_DAMAGE):
                                    if obj.object_type == "boss":
                                        current_scene_obj.completed = True
                                        boss_defeated = True
                                        vasily.money += 15
            ...
            # Вход в дом (магазин) на уровне 2: зона у центра внизу экрана
            if shop_exit_cooldown > 0:
                shop_exit_cooldown -= 1
            if current_scene_obj.name == "level2_house_scene" and not in_shop and shop_exit_cooldown <= 0:
                entrance_left = SCREEN_WIDTH // 2 - 70
                entrance_right = SCREEN_WIDTH // 2 + 70
                entrance_top = SCREEN_HEIGHT - 220
                player_center_x = vasily.x + hero_sprite_width // 2
                player_bottom = vasily.y + hero_sprite_height
                if entrance_left <= player_center_x <= entrance_right and player_bottom >= entrance_top:
                    interaction_hint = "Дом: нажми E чтобы войти в магазин"
                    if keys[pygame.K_e]:
                        in_shop = True
        ...
        if explosion_active:
            ...
            alpha = 255 - (explosion_timer * 8) if explosion_timer < 32 else 0
            if alpha > 0:
                s = pygame.Surface((r * 2, r * 2))
                s.set_alpha(alpha)
                s.fill(ORANGE)
                pygame.draw.circle(s, RED, (r, r), r)
                screen.blit(s, (explosion_x - r, explosion_y - r))
        
        # Магазин внутри дома (уровень 2): комната-магазин, в которую можно зайти
        if in_shop and shop_sprite:
            shop_scaled = pygame.transform.scale(shop_sprite, (SCREEN_WIDTH, SCREEN_HEIGHT))
            screen.blit(shop_scaled, (0, 0))
            shop_font = pygame.font.Font(None, 48)
            welcome_text = shop_font.render("Cheers, home!", True, WHITE)
        ...
        if multiplayer_mode and vasily2 and not in_shop:
            if vasily2.has_sword:
        ...
        # Интерфейс (левый верхний угол: деньги, затем сцена и остальное)
        font = pygame.font.Font(None, 36)
        money_text = font.render(f"Деньги: {vasily.money}", True, BLACK)
        screen.blit(money_text, (10, 10))
        scene_text = font.render(f"Сцена: {scenes[current_scene].name}", True, BLACK)
        screen.blit(scene_text, (10, 32))
        
        if vasily.has_sword:
            sword_text = font.render("Меч: ✓", True, BLACK)
            screen.blit(sword_text, (10, 54))
        else:
